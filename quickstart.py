...
def follow():
    ...
    send_message('InstaPy Follower Started @ {}'.format(datetime.now().strftime("%H:%M:%S")), "728810316")
    ...
    with smart_run(session):
        ...
        while counter < 5:
            ...
            try:
                ...
                # activity
                session.follow_by_tags(['fitness', 'yoga'], amount=10)
                session.follow_user_followers(['gymshark', 'zyiaactive','bootybandsofficial','alphalete','squat_wolf','ryu_apparel','gymtopz_ve','koral','varley','p.e.nation','michi_ny','squats_fitness_apparel','muscleclub','vimmia_active','karma_athletics','heroinesport','brazilwear','rhoneapparel','eysommenswear','wearittoheart','sukishufu','tenthousandgear','krissycela','fitgurlmel','katiesonier','natacha.oceane','brittnebabe'], amount=10, randomize=False)
    ...
    # Send notification to my Telegram
    send_message('InstaPy Follower Stopped @ {}'.format(datetime.now().strftime("%H:%M:%S")), "728810316")


def unfollow():
    send_message('InstaPy Unfollowing Started @ {}'.format(datetime.now().strftime("%H:%M:%S")), "728810316")
    ...
    send_message('InstaPy Unfollower Stopped @ {}'.format(datetime.now().strftime("%H:%M:%S")), "728810316")


def xunfollow():
    send_message('InstaPy Unfollower Wednesday Started @ {}'.format(datetime.now().strftime("%H:%M:%S")), "728810316")
    ...
    send_message('InstaPy Follower Wednesday Stopped @ {}'.format(datetime.now().strftime("%H:%M:%S")), "728810316")
