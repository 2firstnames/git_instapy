    ...
    with smart_run(session):
        ...
        while counter < 5:
            ...
            try:
                ...
                # activity
                send_message('InstaPy Following by Tags @ {}'.format(datetime.now(tz)),"728810316")
                session.follow_by_tags(['fitness', 'yoga'], amount=20)
                send_message('InstaPy Following User Followers @ {}'.format(datetime.now(tz)),"728810316")
                session.follow_user_followers(['gymshark', 'zyiaactive','bootybandsofficial','alphalete','squat_wolf','ryu_apparel','gymtopz_ve','koral','varley','p.e.nation','michi_ny','squats_fitness_apparel','muscleclub','vimmia_active','karma_athletics','heroinesport','brazilwear','rhoneapparel','eysommenswear','wearittoheart','sukishufu','tenthousandgear','krissycela','fitgurlmel','katiesonier','natacha.oceane','brittnebabe'], amount=50, randomize=True)
